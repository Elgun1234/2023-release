...
def ExtractOpCode(Instruction, LineNumber, Memory):
    if len(Instruction) > 9:
        OpCodeValues = ["LDA", "STA", "LDA#", "HLT", "ADD", "JMP", "SUB", "CMP#", "BEQ", "SKP", "JSR", "RTN", "CMP", "   "]
...
def ExecuteCMPD(Registers, Memory, Address):
...
def Execute(SourceCode, Memory):
    ...
    while OpCode != "HLT":
        ...
        if OpCode == "LDA":
            ...
            Registers = ExecuteCMPD(Registers, Memory, Operand)
        elif OpCode == "BEQ":
